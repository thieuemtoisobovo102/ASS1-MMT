...
class Response():   
    ...
    def build_content(self, path, base_dir):
        ...
        try:
            ...
            # Trả về 0 và content rỗng, hàm build_response sẽ xử lý
            return 0, b""
...
#                "Cookie": "{}".format(reqhdr.get("Cookie", "sessionid=xyz789")), #dummy cooki
	...
	# self.auth = ...
        ...
        # Header text alignment
            ...
            #
        # Thiết lập status code và reason mặc định (200 OK)
        if not hasattr(self, 'status_code') or not self.status_code:
            self.status_code = 200
            self.reason = "OK"

        response_line = "HTTP/1.1 {} {}\r\n".format(self.status_code, self.reason)
        
        header_lines = [response_line]
        
        # Thêm các headers từ dictionary
        for key, value in headers.items():
            header_lines.append("{}: {}\r\n".format(key, value))

        # (Task 1) Thêm bất kỳ header Set-Cookie nào đã được đặt
        for key, value in self.new_cookies.items():
            # Cung cấp thêm thuộc tính để bảo mật hơn
            header_lines.append("Set-Cookie: {}={}; HttpOnly\r\n".format(key, value))

        # Thêm header Connection và dòng trống kết thúc
        header_lines.append("Connection: close\r\n")
        header_lines.append("\r\n") # Dòng trống quan trọng
    ...
    def build_response(self, request):
        ...
        #
        elif mime_type.startswith('image/'): # Hỗ trợ tất cả các loại ảnh
            base_dir = self.prepare_content_type(mime_type = mime_type)
        elif path.endswith('.ico'): # Hỗ trợ favicon
        ...
        if c_len == 0 and self._content == b"":
            print(f"[Response] File not found, returning 404 for path: {path}")
            return self.build_notfound()
